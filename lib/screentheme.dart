...
class ScreenTheme {
  ...
  static Future<bool> lightStatusBar({Platform platform = Platform.both}) async {
    bool res = await _channel.invokeMethod('changeStatusBarTheme',
  ...
  static Future<bool> darkStatusBar({Platform platform = Platform.both}) async {
    bool res = await _channel.invokeMethod('changeStatusBarTheme',
        {'theme': 'dark', 'platform': platform.toString()});
    return res;
  }

  static Future<bool> hasALightStatusBar({Platform platform = Platform.both}) async {
    bool res = await _channel
        .invokeMethod('hasLightStatusBar', {'platform': platform.toString()});
    return res;
  }

  static Future<bool> hasANavigationBar({Platform platform = Platform.both}) async {
    bool res = await _channel
  ...
  static Future<bool> updateNavigationBarColor(Color color,
          {Platform platform = Platform.both}) async {
    bool res = await _channel.invokeMethod('changeNavigationBarColor', {
      'red': color.red,
      'blue': color.blue,
      'green': color.green,
  ...
  static Future<bool> lightNavigationBar({Platform platform = Platform.both}) async {
    bool res = await _channel.invokeMethod('changeNavigationBarTheme',
        {'theme': 'light', 'platform': platform.toString()});
    return res;
  }

  /// Will return true only if the status bar was changed (only works on Android 8.0+)
  static Future<bool> darkNavigationBar({Platform platform = Platform.both}) async {
    bool res = await _channel.invokeMethod('changeNavigationBarTheme',
        {'theme': 'dark', 'platform': platform.toString()});
    return res;
  }
